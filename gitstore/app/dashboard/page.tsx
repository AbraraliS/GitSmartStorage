...
export default function DashboardPage() {
  ...
  // Build the filesystem tree once per index change — memoized for performance
  const fileTree = useMemo(() => {
    if (!index) return null;
    return buildFileTree(index);
  }, [index]);

  const computed = useMemo(() => {
    if (!index || !fileTree) return { files: [] as FileRecord[], folders: [] as FolderEntry[] };

    const toFolderEntry = (folderPath: string): FolderEntry => ({
      name:
        index.folders?.[folderPath]?.name ??
        folderPath.split("/").pop() ??
        folderPath,
      path: folderPath,
    });

    // ── Global search (all files, across entire filesystem) ──────────────────
    if (q.trim()) {
      return {
    ...
    // ── Trash ────────────────────────────────────────────────────────────────
    if (view === "trash") {
      const trashed = getTrashedFiles(index);
      trashed.sort(
        (a, b) =>
    ...
    // ── Folder view ──────────────────────────────────────────────────────────
    // Uses the virtual filesystem tree — shows direct children of the folder.
    if (view === "folder" && path) {
      const children = getFolderChildren(fileTree, path);
      const folderEntries: FolderEntry[] = children
        .filter((n) => n.type === "folder")
        .map((n) => ({ name: n.name, path: n.path }));
      const fileRecords = children
        .filter((n) => n.type === "file")
        .map((n) => n.record as FileRecord)
        .filter((f) => !f.trashed);
    ...
    // ALL uploaded files are visible — either inside top-level folders or
    // as root-level files if they have no folder assignment.
    const rootNodes = getRootChildren(fileTree);
    const rootFolderEntries: FolderEntry[] = rootNodes
      .filter((n) => n.type === "folder")
      .map((n) => ({ name: n.name, path: n.path }));
    const rootFileRecords = rootNodes
      .filter((n) => n.type === "file")
      .map((n) => n.record as FileRecord)
      .filter((f) => !f.trashed);

    // Also show root-level files that exist in the index but
    // may not have been picked up by the virtual tree (belt-and-suspenders)
    const treeHashes = new Set(
      rootFileRecords.map((f) => f.hash)
    );
    const orphans = Object.values(index.files).filter(
      (f) =>
        !f.trashed &&
        !treeHashes.has(f.hash) &&
  ...
  const hasLegacyFiles = useMemo(
    () =>
      Object.values(index?.files ?? {}).some(
        (f) => !f.trashed && f.encryptionKey && !f.fixedEncoding
      ),
    [index]
  );
  ...
  const isEmpty =
    computed.files.length === 0 && computed.folders.length === 0;
  ...
  const isMyFilesRoot = !view || (view === "folder" && !path);
  ...
  return (
    <section className="space-y-4">
      {hasLegacyFiles && !bannerDismissed && (
        <div className="flex items-start justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
          <p>
            ...
            A bug in the upload pipeline caused files to be stored with
            corrupted encoding. Delete existing files and re-upload them to fix
              ...
              onClick={() =>
                window.dispatchEvent(
                  new CustomEvent("gitstore:new-upload", {
                    detail: { folder: currentFolderPath },
                  })
                )
              }
          ...
          <div>
            ...
            {fileTree && (
              <p className="mt-0.5 text-xs text-gray-500">
                {fileTree.totalFiles} file
                {fileTree.totalFiles !== 1 ? "s" : ""}
              </p>
            )}
          </div>
          <button
      ...
      {view === "trash" && (
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-xs text-gray-500">
              {computed.files.length} item
              {computed.files.length !== 1 ? "s" : ""}
              {computed.files.length > 0 &&
                " · Files are deleted permanently after 30 days"}
            </p>
          </div>
          {computed.files.length > 0 && (
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={async () => {
                  if (
                    !confirm(
                      `Permanently delete all ${computed.files.length} trashed files? This cannot be undone.`
                    )
                  )
                    return;
                  const hashes = computed.files.map((f) => f.hash);
      ...
      <NewFolderDialog
        ...
        defaultParentPath={
          view === "folder" ? (params.get("path") ?? "/") : "/"
        }
        onConfirm={(createdPath) => {
          setNewFolderDialogOpen(false);
          router.push(
            `/dashboard?view=folder&path=${encodeURIComponent(createdPath)}`
          );
        }}
